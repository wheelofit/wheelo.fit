...
export default function RentalsView({ cycles }: { cycles: CycleData[] }) {
  ...
  const getStartingPriceInfo = (cycle: CycleData) => {
    if (!cycle.pricing || cycle.pricing.length === 0)
      return { price: 0, unit: "MONTHS" };
    const basePricing = cycle.pricing.reduce(
      (min, p) => (p.durationValue < min.durationValue ? p : min),
      cycle.pricing[0],
    );
  ...
  const filteredCycles = useMemo(() => {
    const filtered = cycles.filter((c) => {
      ...
      if (
        selectedTyreSizes.length > 0 &&
        c.tyreSize &&
        !selectedTyreSizes.includes(c.tyreSize)
      )
        return false;
      if (
        selectedSpeeds.length > 0 &&
        c.speed &&
        !selectedSpeeds.includes(c.speed)
      )
        return false;
      if (
        selectedBikeTypes.length > 0 &&
        c.bikeType &&
        !selectedBikeTypes.includes(c.bikeType)
      )
        return false;
      if (
        selectedBrakes.length > 0 &&
        c.brakes &&
        !selectedBrakes.includes(c.brakes)
      )
        return false;
      return true;
    });
  ...
  return (
    <div style={{ maxWidth: "1400px", margin: "0 auto", padding: "2rem 1rem" }}>
              ...
              >
                {[
                  ...
                  <div
                    ...
                    style={{
                      padding: "0.8rem 1rem",
                      color: sortOption === option ? "#1eb53a" : "#fff",
                      cursor: "pointer",
                      background:
                        sortOption === option ? "#222" : "transparent",
            ...
            >
              {tyreSizes.map((ts) => (
                ...
                >
                  <input
                    type="checkbox"
                    checked={selectedTyreSizes.includes(ts)}
                    onChange={() =>
                      toggleArrayItem(
                        selectedTyreSizes,
                        ts,
              ...
              {speeds.map((s) => (
                ...
                >
                  <input
                    ...
                    onChange={() =>
                      toggleArrayItem(selectedSpeeds, s, setSelectedSpeeds)
                    }
                    style={{ width: "16px", height: "16px" }}
                  />
                  {s}
            ...
            >
              {bikeTypes.map((bt) => (
                ...
                >
                  <input
                    type="checkbox"
                    checked={selectedBikeTypes.includes(bt)}
                    onChange={() =>
                      toggleArrayItem(
                        selectedBikeTypes,
                        bt,
            ...
            >
              {brakes.map((b) => (
                ...
                >
                  <input
                    type="checkbox"
                    checked={selectedBrakes.includes(b)}
                    onChange={() =>
                      toggleArrayItem(selectedBrakes, b, setSelectedBrakes)
                    }
                    style={{ width: "16px", height: "16px" }}
                  />
                  {b}
              ...
              >
                ...
                >
                  ...
                  >
                    {cycle.bikeType || "MTB"} |{" "}
                    {cycle.tyreSize
                      ? cycle.tyreSize.split("(")[1]?.replace(")", "") ||
                        cycle.tyreSize
                      : "4'7 to 5'1 ft"}
                  </p>
      ...
      <HeightChartModal
        isOpen={isHeightChartOpen}
        onClose={() => setIsHeightChartOpen(false)}
      />
